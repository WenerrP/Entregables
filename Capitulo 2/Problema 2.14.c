...
void main(void) //Es el punto de entrada al programa
{
    ...
    ejecutar dentro de varias opciones */
    {
        ...
        case 20: COS = TIE * 0.28 / 60; 
            break;
        default : COS = -1; //Para especificar el caso predeterminado en la instrucción switch.
            break; 
    }
    if (COS != -1) //Se utiliza para establece una condicion
        printf("\n\nClave: %d\tTiempo: %d\tCosto: %6.2f", CLA, TIE, COS); /* Si la codicion es verdadera 
        se presentara por panta el mensaje */
    else //Si la condicion es falsa pasa a esta linea de codigo
        printf("\nError en la clave"); /* Si la condicion es falsa se presenta por pantalla el 
        siguiente mensaje */
}