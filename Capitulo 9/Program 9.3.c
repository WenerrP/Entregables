...
void main(void)
{
    ...
    pudo abrir. */
    {
        ...
        while (res)
        {
            ...
            if (res)
            fputs("\n", ar);
            /* Se indica un salto de linea, excepto en la última cadena. Si no
            se hiciera esta indicación, la función fputs pegaria las cadenas y
            luego tendriamos dificultades en el momento de leerlas. Por otra
            parte, si realizáramos este salto de linea al final de la última
            cadena, en la escritura se repetiria la última cadena. */
        }
        fclose(ar);
    }
    else
