...
int main(void) 
{
    ...
    char cad2[] = "Mexico";     /* En este caso se asignan seis caracteres (más
        el caracter de terminación) a la variable cad2. Observa que cad2 no tiene
        espacio     reservado como cad1; por lo tanto, acepta cualquier número de
        caracteres. */
    ...
    char p;
    int i = 0;
    /* La declaración de variables siempre se debe realizar en la parte inicial
    del programa. En este caso se colocan en esta sección (char p e int i = 0)
    para que puedas observar la relación directa con las líneas de programación
    ...
    asignarlos a la variable de tipo cadena de caracteres cad6. Observa que se
    leen caracteres mientras no se encuentre al caracter que indica fin de línea '\n'. */
