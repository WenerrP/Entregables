...
int main(void) 
{
    ...
    strncpy(cad2, cad0, 4);
    cad2[4] = '\0';
    /* La funcion strncpy permite copiar un número determinado de caracteres a otra cadena de caracteres.
    En este caso se copian 4 caracteres de la cadena cad0 —segundo argumento— a cad2 —primer argumento.
    Siempre se debe incorporar al final de la cadena el caracter de terminacion. Si el espacio
    reservado para cad2 es menor que lo que se pretende copiar, se genera un error en la ejecucion del programa. */
    ...
    strcat(cad0, cad3);
    /* La funcion strcat permite incorporar una cadena de caracteres a otra cadena dada. En este caso se
    agrega la cadena cad3 a cad0. Si el espacio reservado para cad0 es menor a lo que se debe almacenar se
    ...
    strncat(cad2, cad0, 4);
    printf("\nPrueba de la funcion strncat. Se incorporan 4 caracteres de cad0 cad2: %s\n", cad2);
    /* La funcion strncat permite incorporar un número determinado de caracteres
    a una cadena. En este caso se agregan cuatro caracteres de la cadena cad0 a cad2.
    Si el espacio de cad2 es menor a lo que se debe almacenar ocurre un error de ejecucion. */
